...
export const AgendaItemRow: React.FC<AgendaItemRowProps> = ({ 
  ...
  const [editRoutinePattern, setEditRoutinePattern] = useState<'weekly' | 'monthly' | 'quarterly' | 'yearly'>(item.routinePattern || 'monthly');
  ...
  const startEditRoutine = () => {
    if (isReadOnly) return;
    setEditRoutinePattern(item.routinePattern || 'monthly');
  ...
  return (
    <div className={`${rowHoverClass} flex flex-col border-b border-gray-200 last:border-0 min-w-[700px]`}>
      <div className={`p-3 grid ${isTemplateMode ? 'grid-cols-[60px_1fr_auto]' : 'grid-cols-[85px_1fr_auto]'} gap-3 items-start`}>
        
        <div className="flex items-center gap-2 pt-0.5">
          {isTemplateMode || isReadOnly ? (
            <span className="font-bold text-gray-500 text-sm ml-2">{index + 1}.</span>
          ) : (
            <>
              <select value={index} onChange={(e) => onMove && onMove(item.id, Number(e.target.value))} className="appearance-none text-center font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 border border-transparent rounded text-sm py-1 px-1.5 cursor-pointer outline-none">
        ...
        <div className="flex flex-col min-w-0 pr-4 pt-1">
          {editField === 'title' ? (
            <input 
            ...
            <div className={`font-bold text-gray-900 text-sm whitespace-normal break-words ${isReadOnly ? '' : 'cursor-text hover:bg-gray-100 rounded px-1 -ml-1 transition-colors'}`} title={isReadOnly ? "" : "Klicken zum Bearbeiten"} onClick={() => startEdit('title', item.title)}>
              {item.title}
            </div>
          )}
          
          <div className="flex items-center gap-2 text-xs text-gray-600 mt-0.5 truncate pl-1">
            {isTemplateMode && item.durationEstimate ? <span className="px-1.5 py-0.5 bg-gray-100 text-gray-600 font-bold rounded">{item.durationEstimate} Min.</span> : null}
            ...
            {item.isRoutine && <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 font-bold rounded">Routine</span>}
          </div>
        </div>

        <div className="flex flex-col">
          <div className="flex items-center shrink-0 pt-1">
            <div onClick={() => !isReadOnly && onEdit(item)} className={`w-[140px] flex items-start pr-4 text-xs text-gray-600 border-r border-transparent ${isReadOnly ? '' : 'cursor-pointer hover:bg-gray-50 rounded -ml-1 pl-1 transition-colors'}`} title={isReadOnly ? "" : "Klicken um Verantwortliche zu ändern"}>
              <User className="w-3 h-3 mr-1.5 shrink-0 text-gray-400 mt-0.5" />
              <span className="truncate">{getAssigneesText(item)}</span>
            </div>
            ...
            <div className="w-[120px] flex justify-start pr-2">
              {isEditingDate && item.type === 'AUFGABE' ? (
                ...
                <div 
                   ...
                   className={`flex items-center text-xs ${isReadOnly ? '' : 'cursor-pointer hover:bg-gray-50 rounded px-1 -ml-1 transition-colors'} ${getDueDateColor(item)}`}
                   title={isReadOnly || item.type !== 'AUFGABE' ? "" : "Klicken zum Bearbeiten"}
                >
                  {item.isDueNextMeeting ? (
                    <span className="flex items-center font-bold bg-purple-50 px-1.5 py-0.5 rounded text-xs"><Calendar className="w-3 h-3 mr-1 shrink-0" /> Nächste</span>
                  ) : item.type === 'AUFGABE' ? (
                    <>
          ...
          {item.isRoutine && (
            <div className="mt-1 flex w-full">
              {isEditingRoutine ? (
                <div className="flex items-center gap-2 bg-indigo-50 px-2 py-1 rounded border border-indigo-200 shadow-sm relative z-10 w-full">
                ...
                <div 
                  ...
                  className={`text-[11px] font-bold text-indigo-600 rounded px-1.5 py-0.5 leading-tight inline-flex items-center -ml-1 ${isReadOnly ? '' : 'cursor-pointer hover:bg-indigo-50 transition-colors'}`}
        ...
        <div className="px-3 pb-3">
          {editField === 'description' ? (
             <textarea 
               ...
               className="w-full text-sm text-gray-900 bg-white p-3 rounded border-2 border-blue-500 outline-none shadow-sm min-h-[80px] overflow-hidden resize-none"
             />
          ) : (
            <div onClick={() => startEdit('description', item.description || '')} className={`text-sm text-gray-700 whitespace-pre-wrap bg-gray-50 p-3 rounded border border-gray-100 shadow-inner ${isReadOnly ? '' : 'cursor-text hover:bg-gray-100 transition-colors'}`} title={isReadOnly ? "" : "Klicken zum Bearbeiten"}>
              {item.description}
            </div>
          )}
        </div>

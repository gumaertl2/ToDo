...
export const ProtocolEditor: React.FC<ProtocolEditorProps> = ({ event, onClose }) => {
  ...
  // CHIRURGISCHER EINGRIFF: Namen für die Anwesenheitsanzeige aufbereiten
  const getNames = (ids?: string[]) => ids ? ids.map(id => users.find(u => u.id === id)?.name).filter(Boolean).join(', ') : '-';
  ...
  return (
    <div className="bg-white rounded-xl shadow border border-gray-200 p-6">
      ...
      {/* CHIRURGISCHER EINGRIFF: Anzeige der protokollierten Anwesenheit im Editor */}
      <div className="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200 text-sm">
        <h3 className="font-bold text-gray-800 mb-2">Anwesenheit ({event.attendanceConfirmed ? 'Bestätigt' : 'Nicht bestätigt'})</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div><span className="font-semibold text-green-700">Anwesend:</span> {getNames(event.actualAttendeeUserIds)}</div>
          <div><span className="font-semibold text-yellow-700">Entschuldigt:</span> {getNames(event.excusedAttendeeUserIds)}</div>
          <div><span className="font-semibold text-red-700">Unentschuldigt:</span> {getNames(event.unexcusedAttendeeUserIds)}</div>
        </div>
      </div>

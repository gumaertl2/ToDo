...
export const CalendarExportModal: React.FC<Props> = ({ onClose, calendarTitle }) => {
  ...
  const handlePrint = () => {
    ...
    // CHIRURGISCHER EINGRIFF: Intelligente Layout-Weiche
    const isOnlyServices = !includeInternal && includeServices;
    ...
    // Das Layout passt sich dem gesetzten Häkchen an!
    if (isOnlyServices) {
      html += `<tr><th style="width: 10%">KW</th><th style="width: 20%">Start</th><th style="width: 20%">Ende</th><th>Dienst</th></tr>`;
    } else {
      html += `<tr><th style="width: 20%">Datum</th><th style="width: 20%">Zeit</th><th>Termin / Dienst</th><th style="width: 20%">Ort</th></tr>`;
    }
    
    html += `</thead><tbody>`;
    
    if (eventsToPrint.length === 0) {
      html += `<tr><td colspan="4" style="text-align:center; padding: 20px;">Keine Einträge in diesem Zeitraum gefunden.</td></tr>`;
    } else {
      eventsToPrint.forEach(ev => {
        const startD = new Date(ev.startTime);
        
        if (isOnlyServices) {
          const endD = ev.endTime ? new Date(ev.endTime) : startD;
          const kw = getISOWeek(startD);
          const startStr = format(startD, 'dd.MM.yyyy', { locale: de });
          const endStr = format(endD, 'dd.MM.yyyy', { locale: de });
          
          html += `<tr><td class="date">${kw}</td><td>${startStr}</td><td>${endStr}</td><td><strong>${ev.title}</strong></td></tr>`;
        } else {
          const dateStr = format(startD, 'dd.MM.yyyy', { locale: de });
          let timeStr = 'Ganztägig';
          
          if (!ev.isAllDay) {
            timeStr = format(startD, 'HH:mm', { locale: de });
            if (ev.endTime) timeStr += ' - ' + format(new Date(ev.endTime), 'HH:mm', { locale: de });
          }
          
          html += `<tr><td class="date">${dateStr}</td><td>${timeStr}</td><td><strong>${ev.title}</strong></td><td>${ev.location || ''}</td></tr>`;
        }
      });
    }

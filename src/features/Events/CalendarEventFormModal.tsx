...
export const CalendarEventFormModal: React.FC<Props> = ({ onClose, existingEvent }) => {
  const { addCalendarEvent, updateCalendarEvent, deleteCalendarEvent, deleteCalendarSeries, users } = useClubStore();
  ...
  const [reminderRecipientUserIds, setReminderRecipientUserIds] = useState<string[]>(existingEvent?.reminderRecipientUserIds || []);
  const [reminderRecipientGroupIds, setReminderRecipientGroupIds] = useState<string[]>(existingEvent?.reminderRecipientGroupIds || []);
  const [reminderRecipientTeamIds, setReminderRecipientTeamIds] = useState<string[]>(existingEvent?.reminderRecipientTeamIds || []);
  const [reminderRecipientHelperIds, setReminderRecipientHelperIds] = useState<string[]>(existingEvent?.reminderRecipientHelperIds || []);
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-6 space-y-4 overflow-y-auto max-h-[75vh] custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2 flex items-center my-1">
              ...
              <label htmlFor="allDay" className="text-sm text-gray-700 font-medium cursor-pointer">Ganztägiger Termin</label>
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Startdatum</label>
              <input type="date" value={startDate} onChange={(e) => handleStartDateChange(e.target.value)} disabled={isSaving} className="w-full p-2 border border-gray-300 rounded" />
            </div>
            {!isAllDay && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Startzeit</label>
                <input type="time" value={startTime} onChange={(e) => handleStartTimeChange(e.target.value)} disabled={isSaving} className="w-full p-2 border border-gray-300 rounded" />
              </div>
            )}
          ...
          <div className="mt-6 bg-green-50/50 border border-green-200 rounded-xl p-4">
            <h3 className="text-sm font-bold text-green-800 mb-4 flex items-center border-b border-green-200/50 pb-2">
              <MessageCircle className="w-5 h-5 mr-2" />
              Omni-Channel Erinnerung (In-App & WhatsApp)
            </h3>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white p-3 rounded-lg border border-green-100 shadow-sm">
                ...
                {/* NEU: Badge-Anzeige für ausgewählten Absender */}
              ...
              {reminderSenderUserId && (
                <>
                  ...
                  <div className="bg-white p-3 rounded-lg border border-green-100 shadow-sm">
                    <label className="block text-xs font-bold text-green-700 mb-2 flex items-center">
                      <Users className="w-3.5 h-3.5 mr-1" /> Wer soll erinnert werden? (Empfänger)
                    </label>
                    <p className="text-[10px] text-gray-500 mb-2 leading-tight">Die Gewählten sehen diese Erinnerung pünktlich in der PapaToDo App, selbst wenn sie kein WhatsApp haben.</p>
                    <div className="border border-gray-200 rounded-lg overflow-hidden">
                      <SmartEntityPicker
                        selections={{
                        ...
                        showBadges={true}
                        placeholder="Teams, Mitglieder oder Ämter suchen..."
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div className="col-span-1">
                      <label className="block text-xs font-bold text-green-700 mb-1">Tage vorher?</label>
                      <input 
                        type="number" min="1" max="365"
                        value={reminderLeadDays} 
                        onChange={(e) => setReminderLeadDays(e.target.value)}
                        disabled={isSaving}
                        className="w-full p-2 text-sm border border-green-300 rounded focus:ring-green-500 bg-white"
                      />
                    </div>
                    <div className="col-span-1 md:col-span-3">
                      <label className="block text-xs font-bold text-green-700 mb-1">Erinnerungstext</label>
                      <textarea 
        ...
        <div className="p-4 border-t border-gray-200 bg-gray-50 flex flex-wrap justify-between gap-3 shrink-0">
          {existingEvent ? (
          ...
          <div className="flex gap-3">
            ...
            {!existingEvent && (
              <button onClick={() => handleSave(true)} disabled={isSaving} className="flex items-center px-4 py-2 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 disabled:opacity-50 font-bold text-sm transition">
                <Plus className="w-4 h-4 mr-2" />
                {isSaving ? 'Lädt...' : 'Speichern & Neu'}
              </button>
            )}

            <button onClick={() => handleSave(false)} disabled={isSaving} className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 font-bold text-sm transition">

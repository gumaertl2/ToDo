...
import { CalendarExportModal } from './CalendarExportModal'; // CHIRURGISCHER EINGRIFF: Neues Export Modal
...
export const CalendarView: React.FC = () => {
  ...
  // CHIRURGISCHER EINGRIFF: Mobile Menu State
  ...
  // CHIRURGISCHER EINGRIFF: Zentrale Datums-Navigation
  const handleNavPrev = () => {
  ...
  const CustomAgendaView = useMemo(() => {
    ...
    View.title = () => ''; // Wir steuern den Titel selbst
    View.navigate = () => new Date(); // Wir steuern die Navigation selbst
    return View;
  }, [hideEmptyDays]);

  const renderActionButtons = () => (
    <>
      <button onClick={() => alert('Wird in Phase 4 gebaut: Generiert den iFrame-Link für die Homepage!')} className="flex items-center w-full lg:w-auto px-3 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-50 transition shadow-sm justify-center">
        <Globe className="w-4 h-4 mr-2 text-blue-500" /> Public Link
      </button>
      <button onClick={() => { setIsMobileMenuOpen(false); setIsExportModalOpen(true); }} className="flex items-center w-full lg:w-auto px-3 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-50 transition shadow-sm justify-center">
        <Printer className="w-4 h-4 mr-2 text-gray-600" /> Export / Druck
      </button>
      <button onClick={() => { setIsMobileMenuOpen(false); setIsSubModalOpen(true); }} className="flex items-center w-full lg:w-auto px-3 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-bold rounded-lg hover:bg-gray-50 transition shadow-sm justify-center">
        <DownloadCloud className="w-4 h-4 mr-2 text-green-500" /> Abos
      </button>
      <button onClick={() => { setIsMobileMenuOpen(false); setSelectedSeriesId(undefined); setIsBulkModalOpen(true); }} className="flex items-center w-full lg:w-auto px-3 py-2 bg-orange-50 border border-orange-200 text-orange-700 text-sm font-bold rounded-lg hover:bg-orange-100 transition shadow-sm justify-center">
        <Settings className="w-4 h-4 mr-2 text-orange-600" /> Dienste
      </button>
      <button onClick={() => { setIsMobileMenuOpen(false); setSelectedEventToEdit(undefined); setIsEventModalOpen(true); }} className="flex items-center w-full lg:w-auto px-4 py-2 bg-blue-600 text-white text-sm font-bold rounded-lg hover:bg-blue-700 transition shadow-sm justify-center">
    ...
    <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-x-4 gap-y-3 p-3 bg-gray-50 lg:bg-white rounded-xl lg:border border-gray-200 lg:shadow-sm">
      ...
      {currentView === 'agenda' && (
        <>
          ...
          <label className="flex items-center space-x-2 text-sm cursor-pointer sm:border-l border-gray-300 sm:pl-4 mt-2 sm:mt-0 w-full sm:w-auto">
    ...
    <div className="h-full flex flex-col space-y-3 relative">
      ...
      <div className="flex items-center justify-between bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-200 z-10 relative">
        <div className="flex-1">
          {isEditingTitle ? (
            <input type="text" value={tempTitle} onChange={(e) => setTempTitle(e.target.value)} onBlur={handleTitleSave} onKeyDown={(e) => e.key === 'Enter' && handleTitleSave()} className="text-xl sm:text-2xl font-bold text-gray-900 border-b-2 border-blue-500 focus:outline-none bg-transparent w-full max-w-sm" autoFocus placeholder="Vereinsname eingeben..." />
          ) : (
      ...
      {/* 2. CHIRURGISCHER EINGRIFF: Die PWA Navigationsleiste */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between bg-white p-2 rounded-xl shadow-sm border border-gray-200 gap-2">
        <div className="flex items-center justify-between sm:justify-start gap-1 flex-1">
          <button onClick={handleNavPrev} className="p-3 sm:p-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-gray-700 transition border border-gray-200"><ChevronLeft className="w-5 h-5"/></button>
          <button onClick={() => setCurrentDate(new Date())} className="p-3 sm:p-2 bg-blue-50 hover:bg-blue-100 rounded-lg text-blue-700 transition border border-blue-100" title="Heute"><Home className="w-5 h-5"/></button>
          <button onClick={handleNavNext} className="p-3 sm:p-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-gray-700 transition border border-gray-200"><ChevronRight className="w-5 h-5"/></button>
          
          {/* Der unsichtbare Datums-Picker Trick */}
          <div className="relative ml-2 sm:ml-4 flex items-center justify-center flex-1 sm:flex-none">
            <span className="font-bold text-gray-800 cursor-pointer text-lg sm:text-base capitalize text-center w-full">
              {getNavLabel()}
            </span>
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg w-full sm:w-auto">
          {['month', 'week', 'day', 'agenda'].map(v => (
            <button key={v} onClick={() => setCurrentView(v as any)} className={`flex-1 sm:flex-none flex items-center justify-center px-2 sm:px-4 py-2 sm:py-1.5 text-xs sm:text-sm font-bold rounded-md transition ${currentView === v ? 'bg-white shadow-sm text-blue-700' : 'text-gray-500 hover:text-gray-800'}`}>
      ...
      {isCalendarLoading ? (
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 flex items-center justify-center text-gray-400 animate-pulse min-h-[400px]">Lade Vereinskalender...</div>
      ) : (
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 p-2 sm:p-3 overflow-hidden flex flex-col min-h-[500px]">
          <Calendar
            culture="de"
            localizer={localizer}
            ...
            endAccessor="end"
            view={currentView}
            toolbar={false} // CHIRURGISCHER EINGRIFF: Die kaputte Standard-Toolbar wird komplett entfernt!
            ...
            className="font-sans text-gray-700 text-xs sm:text-sm"
          />
        </div>
      )}

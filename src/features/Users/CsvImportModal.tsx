// 2026-04-14 12:15 - FIX: Datums-Parser für zweistellige Jahreszahlen (aus CSV)
...
export const CsvImportModal: React.FC<Props> = ({ onClose }) => {
  ...
  // CHIRURGISCHER EINGRIFF: Regex und Logik für 2-stellige Jahreszahlen erweitert
  const parseDate = (dateStr: string) => {
    ...
    if (match) {
      ...
      // Umwandlung von 2-stelligen auf 4-stellige Jahreszahlen
      if (y.length === 2) {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900 flex items-center">
            <Upload className="w-5 h-5 mr-2 text-blue-600" /> Helfer aus CSV importieren
          </h2>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-gray-50 flex flex-col">
          ...
          {previewData.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-blue-200 bg-blue-50/30 rounded-xl p-8 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6 max-w-md">Die Datei sollte eine Kopfzeile mit mindestens "Name" haben. Optional: "Telefon", "Email", "Geburt", "Bezug". (Komma oder Semikolon getrennt).</p>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-100 sticky top-0">
                      <tr>
                        ...
                        <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Name (CSV)</th>
                        <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Ergebnis Alias</th>
                        <th className="px-4 py-2 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Ergebnis Telefon</th>
                      </tr>
                    </thead>
